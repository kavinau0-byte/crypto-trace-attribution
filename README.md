...
7. **Benchmarking Scope & Absence of Accuracy Claims:**  
   See [`benchmarks/BENCHMARK_RESULTS.md`](benchmarks/BENCHMARK_RESULTS.md) for large-scale graph traversal performance and robustness metrics on the Kaggle Elliptic Bitcoin dataset. No VASP-attribution or clustering accuracy number exists or is claimed anywhere in this repository, as the benchmark dataset contains no VASP identity labels or raw UTXO address mappings.
